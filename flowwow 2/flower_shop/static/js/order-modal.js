...
function openOrderModal(productId) {
    ...
    // Ищем товар в разных местах
    let product = null;
    
    // 1. Пробуем через глобальный allProducts из catalog.js
    ...
    // 3. Пробуем через данные из localStorage (если есть)
    if (!product) {
        ...
        if (savedProducts) {
            try {
                ...
                console.log('Найден в localStorage:', product);
            } catch(e) {}
        }
    }
    
    // 4. Если товар не найден, пробуем загрузить его напрямую с сервера
    if (!product) {
        console.log('Товар не найден в кэше, загружаем с сервера...');
        fetchProductFromServer(productId).then(fetchedProduct => {
            if (fetchedProduct) {
                openOrderModalWithProduct(fetchedProduct);
            } else {
...
// Загрузка товара с сервера по ID
async function fetchProductFromServer(productId) {
    try {
        const response = await fetch(`${API_BASE_URL}/products/${productId}/`);
        if (response.ok) {
            const product = await response.json();
            console.log('Товар загружен с сервера:', product);
            return product;
        }
    } catch (error) {
...
function openOrderModalWithProduct(product) {
    console.log('Открытие формы заказа для товара:', product);
