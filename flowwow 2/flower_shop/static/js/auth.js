// дефолтная аватарка
const DEFAULT_AVATAR = 'https://i.pinimg.com/736x/97/78/33/9778339cf8a1e1e1851e6b6ed4ce81c6.jpg';
...
function updateHeaderAuth() {
    ...
    if (isLoggedIn()) {
        ...
        if (user?.avatar && user.avatar !== 'null' && user.avatar !== '') {
            ...
            } else if (user.avatar.startsWith('/media/')) {
                avatarUrl = user.avatar;
        ...
        userAvatar.innerHTML = `
            <img class="user-avatar-img" src="${avatarUrl}" alt="avatar" onerror="this.src='${DEFAULT_AVATAR}'">
...
function openProfileModal() {
    ...
    if (modalAvatarPreview) {
        ...
        modalAvatarPreview.src = avatarUrl;
    }
...
async function saveProfile() {
    if (!currentPasswordInput || !newPasswordInput || !confirmPasswordInput || !saveProfileBtn) {
        console.log('Profile modal elements not found on this page');
        return;
    }
    ...
    try {
        ...
        if (passwordChanged) {
            setTimeout(() => {
                showToast(`Для продолжения войдите снова`);
                logout();
            }, 1000);
        } else {
            window.location.reload();
