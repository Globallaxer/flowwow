// дефолтная аватарка
const DEFAULT_AVATAR = '/static/images/flower.png';
...
async function saveProfile() {
    ...
    if (hasPasswordChange) {
        if (!currentPwd) {
            if (typeof showToast === 'function') showToast(`Введите текущий пароль`);
            return;
        }
        if (newPwd !== confirmPwd) {
            if (typeof showToast === 'function') showToast(`Новые пароли не совпадают`);
            return;
        }
        if (newPwd.length < 4) {
            if (typeof showToast === 'function') showToast(`Новый пароль должен содержать не менее 4 символов`);
            return;
        }
    }
    ...
    try {
        ...
        if (hasPasswordChange) {
            ...
            if (!passwordChanged) {
                if (typeof showToast === 'function') showToast(`Текущий пароль неверный`);
        ...
        if (typeof showToast === 'function') showToast(`Профиль успешно обновлен`);
        closeProfileModal();
        if (passwordChanged) {
            setTimeout(() => {
                if (typeof showToast === 'function') showToast(`Для продолжения войдите снова`);
                logout();
            }, 1000);
        } else {
            window.location.reload();
        ...
        if (typeof showToast === 'function') showToast(`Произошла ошибка при сохранении`);
    } finally {
        saveProfileBtn.disabled = false;
        saveProfileBtn.textContent = 'Сохранить';
    }
...
if (avatarUpload) {
    avatarUpload.addEventListener('change', (e) => {
        ...
        if (file && file.size <= 5 * 1024 * 1024) {
            ...
            if (typeof showToast === 'function') showToast(`Файл не должен превышать 5 mb`);
        }
    });
}
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                updateHeaderAuth();
                closeAuthModal();
                if (typeof showToast === 'function') showToast(`Добро пожаловать, ${username}!`);
                window.location.reload();
            } else {
    ...
    registerForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                updateHeaderAuth();
                closeAuthModal();
                if (typeof showToast === 'function') showToast(`Регистрация прошла успешно! Добро пожаловать, ${username}`);
                window.location.reload();
            } else {
                const error = await response.json();
